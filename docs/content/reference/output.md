...
describes what users get. Pick a format with `-o`, or let rcsb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rcsb <command> -o table   # aligned columns for reading
rcsb <command> -o jsonl   # one JSON object per line, for piping
rcsb <command> -o json    # a single JSON array
rcsb <command> -o csv     # spreadsheet friendly
rcsb <command> -o tsv     # tab-separated
rcsb <command> -o url     # just the URL column
rcsb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rcsb <command> --fields id,title,url
```
...
```bash
rcsb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rcsb <command>            # a table, because this is a terminal
rcsb <command> | wc -l    # JSONL, because this is a pipe
```
